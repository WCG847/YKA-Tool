bl_info = {
    ...
    "category": "Animation",
    "author": "WCG847",
    "version": (4, 9, 1, 1),  # Major, Minor, Small Fixes, Non-Essential Fixes
...
class YKAExporter(bpy.types.Operator):
    ...
    def execute(self, context):
        if not self.filepath or not self.filepath.endswith(".yka"):
            self.report({'ERROR'}, "Filepath not set or invalid extension, set a valid .yka file path")
            return {'CANCELLED'}
    ...
    def calculate_global_root_position(self, armature_obj, root_bone):
        ...
        scale_factor = 100.0  # For now, let's assume Blender uses meters, and PSX uses centimeters
    ...
    def export_yka(self, context):
        ...
        try:
                    ...
                    if obj.type == 'ARMATURE':
                        ...
                        if root_bone:
                            for frame in range(scene.frame_start, scene.frame_end + 1):
                                ...
                                root_pos = self.calculate_global_root_position(obj, root_bone)
            ...
            self.report({'INFO'}, f"Yuke's Keyframe Animation exported to {self.filepath}")
            return {'FINISHED'}

        except FileNotFoundError:
            self.report({'ERROR'}, "Invalid filepath. Please check if the directory exists.")
            return {'CANCELLED'}
    ...
    def export_frame(self, obj, root_pos, file, frame, scene):
        ...
        if obj.type == 'ARMATURE':
            ...
            num_joints = 0
            dynamic_scale, positional_data_size = self.calculate_dynamic_scale_and_size(root_pos)

            # Add positional data size to num_joints (to include X, Y, Z)
            num_joints += positional_data_size

            for bone in bones:
                num_joints += 3   # 3 bytes per bone for rotational data (yaw, pitch, roll)
            ...
            if frame_size > 255:
                self.handle_frame_overflow(frame_size, num_joints, root_pos, bones, file, dynamic_scale, frame, scene)
            else:
                # Write the adjusted frame data
                self.write_frame_data(file, frame_size, num_joints, root_pos, bones, dynamic_scale, frame, scene, flag_data, sprite_data, sound_data)
        ...
        # Camera and Lights are placeholders. needs precise logic that the game expects.
            ...
            file.write(struct.pack('B', 0x80 + frame_size)) # Yuke's for some reason adds 0x80 to the actual frame length.
            ...
            dynamic_precision = 255
            num_joints = 6 + 6 + 4   # 6 bytes for position, 6 bytes for rotation, 4 bytes for intensity. Intensity isnt fully confirmed. 
            flag_data, flag_size = self.pack_frame_flags(frame, scene)
            num_joints += flag_size
            frame_size = num_joints + 1
    ...
    def calculate_precision(self, frame, scene, root_position):
        ...
        max_range = max(abs(root_position.x), abs(root_position.y), abs(root_position.z))
        ...
        if max_range > 1.0:
            ...
            precision = 255 / (max_range + 0.001)  # Scale precision for smaller movements
        else:
        ...
        print(f"Frame: {frame}, Dynamic Precision (based on root position): {precision}")
        return precision
    ...
    def pack_positional_data(self, location, scale_factor, positional_data_size):
        ...
        y_quantised = quantise_pos(location.y, yuke_reference[2])  # Using appropriate Yuke's byte for Y axis
        z_quantised = quantise_pos(location.z, yuke_reference[4])  # Using appropriate Yuke's byte for Z axis

        # Pack positional data based on the byte size (6 bytes)
        return struct.pack('BBBBBB', x_quantised, yuke_reference[1], y_quantised, yuke_reference[3], z_quantised, yuke_reference[5])
    ...
    def handle_frame_overflow(self, frame_size, num_joints, root_pos, bones, file, dynamic_precision, frame, scene):
        ...
        while remaining_frame_size > 255:
            ...
            self.write_sub_frame(file, sub_frame_size, num_joints, root_pos, bones, dynamic_precision, frame, scene)
            remaining_frame_size -= sub_frame_size

    def write_sub_frame(self, file, frame_size, num_joints, root_pos, bones, dynamic_precision, frame, scene):
        # Write the sub-frame data similar to a full frame, but smaller chunks
        file.write(struct.pack('B', 0x80 + frame_size))  # Yukes adds 0x80 to frame length for some reason
    ...
    def write_frame_data(self, file, frame_size, num_joints, root_pos, bones, dynamic_precision, frame, scene, flag_data, sprite_data, sound_data):
        ...
        if root_pos:
            ...
            pos_data = self.pack_positional_data(root_pos, dynamic_scale, positional_data_size)
            file.write(pos_data)
        else:
...
class YKA_PT_MarkersPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.template_list("UI_UL_list", "yka_markers", context.scene, "yka_markers", context.scene, "yka_markers_index")
        ...
        try:
            with open(self.filepath, 'rb') as f:
                ...
                for anim_idx in range(num_animations):
                    ...
                    print(f"Animation {anim_idx+1}: Part ID: {part_id}, Asset ID: {asset_id}, Frames: {actual_frame_count}, TOC offset: {toc_offset}")
                    ...
                    'L_Thigh': {
                        ...
                        'children': {
                            'L_Knee': {
                                ...
                                'children': {
                                    'End_L_Thigh': {'offset': (1.125349, 4.199849, -0.000001)}
                                }
                            }
                        }
                    },
                    'R_Thigh': {
                        ...
                        'children': {
                            'R_Knee': {
                                ...
                                'children': {
                                    'End_R_Thigh': {'offset': (-1.125185, 4.199890, 0.000000)}
                                }
                            }
                        }
                    },
                    ...
                    'Spine': {
                        ...
                        'children': {
                            'Neck': {
                                ...
                                'children': {
                                    'End_Neck': {'offset': (0.000826, -2.761086, 0.000000)}
                                }
                            }
                        }
                    }
